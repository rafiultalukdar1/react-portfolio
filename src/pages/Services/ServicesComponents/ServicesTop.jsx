            ...
            <div className='pt-20 sm:pt-25 md:pt-28 lg:pt-32'>
                <div className='container'>
                    ...
                    <div className='mt-15 sm:mt-22 md:mt-25 lg:mt-32'>
                        <h2 data-aos="fade-right" class="font-rajdhani text-white text-[25px] sm:text-[30px] md:text-[35px] lg:text-[40px] font-bold capitalize">What I Provide!</h2>
                                ...
                                <div key={service.id} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} className='hover-card hover-card-service px-3.75 sm:px-8 py-3.75 sm:py-8 rounded-xl group'>
                                    <div className='flex flex-col gap-2.5 sm:gap-3.75'>
                                        <div className='flex justify-between items-center'>
                                            <h4 data-aos="fade-right" className='text-4xl text-[#5c59598e] italic font-rajdhani font-bold transition-colors duration-200 group-hover:text-[#fdb500af]'>{service.id}</h4>
                                            <div data-aos="fade-left" className='h-9 w-9 flex items-center justify-center border border-[#FDB400] rounded-full transition-colors duration-200 group-hover:bg-[#FDB400]'>
                                                <IoMdArrowDown className="text-[#FDB400] group-hover:text-black text-[20px] transition-colors duration-200" />
                                            </div>
                                        </div>
                                        <div>
                                            <h2 data-aos="fade-up" data-aos-delay="100" className='text-[22px] sm:text-[26px] text-white font-rajdhani font-semibold'>{service.title}</h2>
                                            <p data-aos="fade-up" data-aos-delay="200" className='font-titillium text-color text-[15px] max-w-110'>{service.description}</p>
                                        </div>
                                        <div className='flex items-center gap-2.5' data-aos="fade-up" data-aos-delay="300">
                                        ...
                                        <div>
                                            <ul className='flex flex-col gap-1'>
                                                {service.scope.map((item, index) => (
                                                    <li key={index} data-aos="fade-up" data-aos-delay={400 + index * 100} className='flex items-center gap-2 text-color font-titillium text-[15px]'><span className='text-[#FDB400]'><MdOutlineKeyboardDoubleArrowRight /></span>{item}</li>
                                                ))}
                                            </ul>
                                        </div>
                                    </div>
